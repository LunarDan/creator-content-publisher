...
@bp.post('/api/publish/xiaohongshu/browser')
def publish_xiaohongshu_with_browser():
    data = request.get_json(force=True)
    draft_id = data.get('platform_draft_id')
    if not draft_id:
        return jsonify({'code': 400, 'msg': '缺少平台草稿ID'}), 400

    result, error = service.publish_xiaohongshu_with_browser(draft_id, auto_publish=False)
    if error:
        status = 404 if error == '平台草稿不存在' else 400
        return jsonify({'code': status, 'msg': error, 'data': result}), status
    return jsonify({'code': 200, 'data': result})
